pro hs_gems_checkarea_ver2,xdrmod=xdrmod,which_avgk=which_avgk,which_plot=which_plot
...
;+---------------------------------------------------------------------------+
  ...
  levs = ['cfraclev','totallev', 'totallev2','cfraclev','totallev3', $
          ...
          'troplev' ,'troplev' , 'ctplev'   ,'ctplev'  ,'ctplev']
  void = execute('lev=' + levs[which_plot])
  
  datalist = ['cfrac'   ,'co3[0,*]' ,'co3[0,*]','cfrac'   ,'co3[0,*]', $
              'cfrac'   , 'co3[1,*]','co3[2,*]','co3[1,*]','co3[1,*]', $
              'co3[2,*]','co3[2,*]' , 'ctp'    ,'ctp'     ,'ctp']
...
;copypres = exp((alog(data.pres[*,1:*]) + alog(data.pres[*,0:-1]))*0.5)
;yrange=[1000,1]
;opl3 = contour(data.ao3[orbitind2,*],xx,copypres[orbitind2,*], &
        ;max_value=50,min_value=0,pos=posi3 $
        ;,xstyle=1,yrange=[1000,0],axis_style=4,/fill,rgb_table=ct,n_levels=21,/current)
