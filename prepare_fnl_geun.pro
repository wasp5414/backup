...
;+---------------------------------------------------------------------------+
; Sub Procedure 2 : read_fnl_nc
;+---------------------------------------------------------------------------+
PRO sub2_read_fnl_nc, date, utc, fnl, fnl_new, get_rearray=get_rearray

  dir   = '/home/Data/FNL/'+strmid(date, 0, 4) + '/'
  ...
  fnl   = replicate(str,nutc)
  ;ncdf_cat, file(0)

  FOR k = 0 , nutc-1 do begin
      fname = file(k)
...
;   Output : fnlout 
;+---------------------------------------------------------------------------+
PRO prepare_fnl_geun, date, fnlout, lsttime=lsttime
  ...
  fnltime = [0, 6, 12, 18 , 24]

  ;1) loading fnl data per day
  sub2_read_fnl_nc, date, fnltime, fnl1
