pro hs_analysis,inputday,xdrmod=xdrmod
;+---------------------------------------------------------------------------+
; RUN : hs_analysis,xdrmod=1
...
orbits=[] & pixcor_files=[]
O3  =[] & alt =[] & pres =[] & avgk =[] & ao3  =[] & ao3e =[] & co3 =[]  
ctp =[] & lat =[] & lon  =[] & sza  =[] & cfrac=[] & exval=[] & rms =[]
year=[] & mon =[] & day  =[] & utc =[] & orbs =[] & pixel=[] & tline=[]
line=[] & vza =[]
...
for i=0,fnum-1 do begin   ;read pixcor files, but not used for now.
  ...
  fpix   = [ fix(strpix[0]),fix(strpix[1]) ]
  fline = [ fix(strline[0]),fix(strline[1]) ]
...
for i=0,fnum-1 do begin
  ...
  o3    = [o3   ,bad2nan(transpose(gems.o3))]
  alt   = [alt  ,bad2nan(transpose(gems.alt))]
  pres  = [pres ,bad2nan(transpose(gems.pres))]
  avgk  = [avgk ,bad2nan(transpose(gems.avgk))]
  ao3   = [ao3  ,bad2nan(transpose(gems.ao3))]
  ao3e  = [ao3e ,bad2nan(transpose(gems.ao3e))]
  co3   = [co3  ,bad2nan(transpose(gems.co3))]
  ctp   = [ctp  ,bad2nan(gems.ctp)]
  sza   = [sza  ,bad2nan(gems.sza)]
  cfrac = [cfrac,bad2nan(gems.cfrac)]
  ...
  orb   = intarr(n_elements(gems.day))
  orb[*] = orbits[i] & orbs = [orbs,orb]
...
data={ O3 :o3 ,alt:alt,pres :pres ,avgk :avgk ,ao3:ao3,ao3e:ao3e,co3:co3,$
       ctp:ctp,lat:lat,lon:lon,sza:sza,vza:vza,cfrac:cfrac,exval:exval,$
       rms:rms,year:year,mon:mon,day:day,utc:utc,orbit:orbs,pixel:pixel,$
       line:line}

;orbitind1  = where(data.orbit eq orbits[0] and data.pixel eq 1 ) 
;orbitind2  = where(data.orbit eq orbits[1] and data.pixel eq 15)
;orbitind3  = where(data.orbit eq orbits[2] and data.pixel eq 15)
;orbitind4  = where(data.orbit eq orbits[3] and data.pixel eq 28)
