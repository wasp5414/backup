...
FOR id=0,0 DO BEGIN 
  date=dates[id]
  call_xdr=2  ; 0: run without xdr,  1: xdr generation, 2: use xdr
  ...
  IF call_xdr LE 1 THEN BEGIN
    ...
    omifiles=FILE_SEARCH(omipath+year+'/'+'OMI-Aura*'+year+'m'+ $
    mon+day+'*.he5', count=nfiles)
    omilons =[] &  omilats=[] & omitozs=[] & omitimes=[]  
    omipixs =[] & omilines=[] & pixlons=[] &  pixlats=[]  
    ...
    FOR ifile=0,nfiles-1 DO BEGIN
      ...
      IF ntmp NE 0 THEN BEGIN 
        ...
        pixfile=FILE_SEARCH('/home/Data/OMI/2_OML2PIXCOR/'+year+ $
        '/'+header+time+'*.he5', count=npixfile)
...
;        omiline=omires.lines[tmp]
        ...
        pixlon=pixlon[tmp,*] & pixlat=pixlat[tmp,*]
;        omitime=pixres.jtime[tmp]
    ...
    mod03files=FILE_SEARCH(mod03path+'MYD03.A'+year+julmod+'*.hdf', $
    count=nmod03file)
   ...
   ENDIF
    ...
    FOR ifile=0,nmodfile-1 DO BEGIN
      ...
      mod03res=READ_MODIS_MYD03(mod03file)

      mlon  = modres.lon   & mlat = modres.lat
      hlon  = mod03res.lon & hlat = mod03res.lat
    ...
    omivars={lon:omilons, lat:omilats, time:omitimes, toz:omitozs,$
             ...
             cf:omicfs, pcld:omipclds, qf:omiqfs, bloz:omiblozs}

;ref331:omir331s,

    modvars={lon:modlons, lat:modlats, time:modtimes, ctp:modctps, $
             cf:modcfs, cphase:modcphases, hlon:mod03lons, $
             hlat:mod03lats, htime:mod03times, cot:mod03cots}

    IF call_xdr EQ 1 THEN BEGIN
      SAVE, file=xdrpath+xdrfile, omivars, modvars, /xdr
    ENDIF ; call_xdr
...
; 5km spatial resolution
    ...
    IF ntmp NE 0 THEN BEGIN
      ...
      IF nval NE 0 THEN BEGIN
        dists=ABS(omivars.lon[ipo] - modvars.lon[tmp[val]])+ $
              ABS(omivars.lat[ipo] - modvars.lat[tmp[val]])

        mindist=WHERE(dists EQ MIN(dists), nmindist)
        mindist=mindist[0]
        mdist=MAP_2POINTS(omivars.lon[ipo], omivars.lat[ipo], $
              modvars.lon[tmp[val[mindist]]],modvars.lat[tmp[val[mindist]]], $
              /meters)/1000.
...
;  ENDIF
  ...
  ENDFOR  ; ipix
  res={omi:omivars, omictp:omictp, omicf:omicf, omicphase:omicphase, $
       omicot:omicot}
